...
import { ClientProfileSchema, type ClientProfile, type Goal } from "./schema.js";
...
const GOAL_TYPES = ["retirement", "college", "home", "charitable", "other"] as const;
...
const FILING_STATUSES = ["single", "marriedJoint", "marriedSeparate", "headOfHousehold"] as const;
...
export function generate(opts: GenerateOpts): ClientProfile {
  ...
  const spec: ProfileSpec = opts.profile ? PROFILES[opts.profile] : { label: "" };
  ...
  const annualIncome = snap(spec.annualIncome ?? randFloat(rng, 50_000, 450_000), 1_000);
  const annualSpending = snap(annualIncome * randFloat(rng, 0.55, 0.9), 1_000);
  const liquidAssets = snap(randFloat(rng, annualIncome * 0.3, annualIncome * 2.5), 1_000);

  const totalInvestments =
    spec.targetAssets ?? snap(randFloat(rng, annualIncome * 0.5, annualIncome * 15), 1_000);
  const investmentAssets = allocate(rng, totalInvestments, spec.taxableTilt);

  const hasRealEstate = rng() > 0.3;
  const realEstateValue = hasRealEstate ? snap(randFloat(rng, 200_000, 2_000_000), 10_000) : 0;
  const mortgageBalance =
    hasRealEstate && rng() > 0.2 ? snap(realEstateValue * randFloat(rng, 0.1, 0.7), 10_000) : 0;
  const nonMortgageDebt = rng() > 0.5 ? snap(randFloat(rng, 1_000, 50_000), 1_000) : 0;

  const goals: Goal[] = spec.goals ?? randomGoals(rng);

  const retirementAge = spec.retirementAge ?? randInt(rng, age + 3, Math.min(age + 40, 80));
  const targetRetirementIncome = snap(annualIncome * randFloat(rng, 0.5, 0.85), 1_000);

  const riskScore = spec.riskScore;
  const riskTolerance = riskScore != null
    ? riskScoreToTolerance(riskScore)
    : spec.riskTolerance ?? pick(rng, RISK_LEVELS);
  const timeHorizonYears = spec.timeHorizonYears ?? Math.max(1, retirementAge - age);
