...
| Path                           | `POST /mcp/tools/{toolId}` where `{toolId}` is one of the five ids in §3.                                                                                                                                                                                                                                                                                     |
...
The HTTP client hardcodes the five tool ids and does **not** call `tools/list` at runtime — but for MCP correctness and stdio/agent use:

1. **Expose all five as standard MCP tools** (`tools/list` + `tools/call`) with JSON Schemas matching §3. The HTTP gateway maps `/mcp/tools/{id}` → `tools/call`.
...
- `pathCacheKey` — reference to an engine-side path cache. **See §5 gap:** the contract says "pass to `monte_carlo` to reuse," but `MonteCarloRequest` has no field to carry it yet.
...
1. **Public, unauthenticated, CORS-enabled access** to all five tools from the browser (see §1). This is the single hardest requirement and the one most likely to be missed.
...
3. **"Real data, fake clients" mode** _(new capability to add)_. For case studies the demo wants real capital-market assumptions but fake portfolios. Provide a way to source `expectedReturn` / `volatility` / `lambda` / `correlations` from the engine's own market data rather than client-supplied numbers — e.g.:
   - a **capital-market-assumptions endpoint** returning a starter asset universe (`AssetClass[]`) + correlation matrix + `asOf` date, and/or
   - a flag on `monte_carlo_decumulation` to fill missing per-asset params from engine data.
     This is what lets demos say "these are real return assumptions as of {date}."
...
## 6. Gaps / contract changes to coordinate

These are mismatches between the current contract and the engine behavior implied by it. Resolve before/with the engine build (all are additive → minor bumps):

1. **`pathCacheKey` has no consumer field.** `regime_return_generator` returns `pathCacheKey` "to pass to `monte_carlo` to reuse," but `MonteCarloRequest` has no field to accept it. If you want the cache-reuse flow, add `pathCacheKey?: string` to `MonteCarloRequest` (additive, minor bump).
2. **`correlation_matrix` must back MC's auto-estimation.** MC says it estimates correlations when the client omits them — so the engine's MC path must internally use the same estimator the `correlation_matrix` tool exposes.
3. **No capital-market-assumptions / "real data" endpoint exists** — needed for §5.3. Propose adding (new tool, additive).
4. **No `tools/list` / health / version handshake is consumed yet** — recommend the engine expose them anyway (§2.3–2.4) for robustness and proactive mismatch warnings.
